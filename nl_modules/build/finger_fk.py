...
class FingerFk(RigModule):
    ...
    def build_ctl(self):
        ...
        ctl_defs = [
            ...
            ("fgr01_fkc", "sphereL", "x", scale, 1, 2),
            ("fgr02_fkc", "sphereL", "x", scale, 1, 2),
            # ("ikc", "cube", None, scale, 1, 2),
            # ("extra_rota", "rotator", None, scale, 0, -1),
            # ("pvc", "pvc", "z", -scale, 0, -1),
        ]
        if self.segNum >= 2:
            ctl_defs.append(("fgr03_fkc", "sphereL", "x", scale, 1, 2))
        if self.segNum >= 3:
            ctl_defs.append(("fgr04_fkc", "sphereL", "x", scale, 1, 2))
        if self.segNum >= 4:
            ctl_defs.append(("fgr05_fkc", "sphereL", "x", scale, 1, 2))
